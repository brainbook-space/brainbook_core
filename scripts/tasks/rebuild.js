...
const MODULES_NEEDING_REBUILD = ['sqlite3','better-sqlite3']
...
gulp.task('rebuild', gulp.series(async () => {
  ...
  for (let mod of MODULES_NEEDING_REBUILD) {
    await runAsync(`npm rebuild ${mod} --runtime=electron --target=20.2.0 --disturl=https://electronjs.org/headers --build-from-source`, {cwd, env, shell: true})
  }
